    ...
    void influnceMembers(){
    	...
    	for (int x = 0; x<independentAverage.size(); x++){
    		...
    		for (int y = 0; y<students.size(); y++){
    			independentTemp+=students.get(y).getIndependentAttributes().get(x);
    			dependentTemp+=students.get(y).getDependentAttributes().get(x);
    		}
    		independentAverage.set(x, (independentTemp/students.size()));
    		dependentAverage.set(x, (dependentTemp/students.size()));
    	}
